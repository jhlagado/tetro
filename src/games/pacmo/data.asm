PACMO_WORLD_MAX: EQU    14
PACMO_VIEW_MAX:  EQU    7
PACMO_MOVE_PERIOD: EQU   128
PACMO_EATEN_BYTES: EQU  30
PACMO_POWER_PILL_COUNT: EQU 4
PACMO_POWER_TIMER_START: EQU 0x0600
PACMO_POWER_WARNING_BLINK_MASK: EQU 0x20
PACMO_GAME_OVER_GATE_TICKS: EQU 0x0266
PACMO_LEVEL_COMPLETE_GATE_TICKS: EQU 0x0300
PACMO_LIVES_START: EQU 3
PACMO_SCORE_PATH: EQU 10
PACMO_SCORE_POWER: EQU 50
PACMO_SCORE_ENEMY: EQU 200
PACMO_SOUND_POWER_LEN: EQU 64
PACMO_SOUND_POWER_DIV: EQU 1
PACMO_SOUND_EAT_ENEMY_LEN: EQU 96
PACMO_SOUND_EAT_ENEMY_DIV: EQU 4
PACMO_SOUND_CAUGHT_LEN: EQU 240
PACMO_SOUND_CAUGHT_DIV: EQU 10
PACMO_SOUND_LEVEL_COMPLETE_LEN: EQU 112
PACMO_SOUND_LEVEL_COMPLETE_DIV: EQU 2
PACMO_COLOR_WALL: EQU COLOR_BLUE
PACMO_COLOR_PATH: EQU COLOR_GREEN
PACMO_COLOR_PLAYER: EQU COLOR_YELLOW
PACMO_COLOR_POWER_PILL: EQU COLOR_WHITE
PACMO_COLOR_ENEMY_ATTACK: EQU COLOR_RED
PACMO_COLOR_ENEMY_FLEE: EQU COLOR_MAGENTA
PACMO_COLOR_GAME_OVER: EQU COLOR_RED
PACMO_COLOR_CAUGHT_WALL: EQU COLOR_RED
PACMO_COLOR_COMPLETE_WALL: EQU COLOR_WHITE
PACMO_COLOR_ROUND_COMPLETE: EQU COLOR_WHITE
PACMO_ENEMY_Y: EQU 13
PACMO_ENEMY_MIN_X: EQU 1
PACMO_ENEMY_MAX_X: EQU 13
PACMO_ENEMY_PERIOD: EQU 128
PACMO_ENEMY_PERIOD_MIN: EQU 56
PACMO_ENEMY_PERIOD_STEP: EQU 8
PACMO_ENEMY_RESPAWN_PERIOD: EQU 192
PACMO_ENEMY_RESPAWN_DIV: EQU 3
PACMO_ENEMY_STATE_ATTACK: EQU 0
PACMO_ENEMY_STATE_FLEE: EQU 1
PACMO_ENEMY_STATE_RESPAWN: EQU 2
PACMO_MONSTER_COUNT: EQU 3
MONSTER_X: EQU 0
MONSTER_Y: EQU 1
MONSTER_DIR: EQU 2
MONSTER_TIMER: EQU 3
MONSTER_RESPAWN_TIMER: EQU 4
MONSTER_STATE: EQU 5
MONSTER_SIZE: EQU 6
PACMO_DIR_UP:    EQU    1
PACMO_DIR_DOWN:  EQU    2
PACMO_DIR_LEFT:  EQU    3
PACMO_DIR_RIGHT: EQU    4

PACMO_KEY_1: EQU 0x01
PACMO_KEY_2: EQU 0x02
PACMO_KEY_3: EQU 0x03
PACMO_KEY_5: EQU 0x05
PACMO_KEY_6: EQU 0x06
PACMO_KEY_7: EQU 0x07
PACMO_KEY_A: EQU 0x0A

LCD_TEXT_PACMO_TITLE:
        DB      "PACMO",0

LCD_TEXT_PACMO_START:
        DB      "PRESS ANY KEY",0

LCD_TEXT_PACMO_KEYS1:
        DB      "ARROWS OR KEYPAD",0

LCD_TEXT_PACMO_KEYS2:
        DB      "6/A UP  2 DOWN",0

LCD_TEXT_PACMO_RUNNING:
        DB      "PACMO RUNNING",0

LCD_TEXT_PACMO_PAUSED:
        DB      "PACMO PAUSED",0

LCD_TEXT_PACMO_POWER:
        DB      "POWER MODE",0

LCD_TEXT_PACMO_ENEMY_EATEN:
        DB      "ENEMY EATEN",0

LCD_TEXT_PACMO_LEVEL:
        DB      "LEVEL ",0

LCD_TEXT_PACMO_LIVES:
        DB      "LIVES ",0

LCD_TEXT_PACMO_CAUGHT:
        DB      "PACMO CAUGHT",0

LCD_TEXT_PACMO_GAME_OVER:
        DB      "GAME OVER",0

LCD_TEXT_PACMO_COMPLETE:
        DB      "LEVEL COMPLETE",0

LCD_TEXT_PACMO_WAIT:
        DB      "WAIT...",0

PACMO_LEVEL_CHAR_TABLE:
        DB      "0123456789ABCDEF"

SCRIPT_PACMO_SPLASH:
        DB      LCD_ROW1
        DW      LCD_TEXT_PACMO_TITLE
        DB      LCD_ROW2
        DW      LCD_TEXT_PACMO_START
        DB      LCD_ROW3
        DW      LCD_TEXT_PACMO_KEYS1
        DB      LCD_ROW4
        DW      LCD_TEXT_PACMO_KEYS2
        DB      0

SCRIPT_PACMO_RUNNING:
        DB      LCD_ROW1
        DW      LCD_TEXT_PACMO_RUNNING
        DB      LCD_ROW2
        DW      LCD_TEXT_PACMO_LEVEL
        DB      0

SCRIPT_PACMO_PAUSED:
        DB      LCD_ROW1
        DW      LCD_TEXT_PACMO_PAUSED
        DB      LCD_ROW2
        DW      LCD_TEXT_PACMO_LEVEL
        DB      0

SCRIPT_PACMO_POWER:
        DB      LCD_ROW1
        DW      LCD_TEXT_PACMO_POWER
        DB      LCD_ROW2
        DW      LCD_TEXT_PACMO_LEVEL
        DB      0

SCRIPT_PACMO_ENEMY_EATEN:
        DB      LCD_ROW1
        DW      LCD_TEXT_PACMO_ENEMY_EATEN
        DB      LCD_ROW2
        DW      LCD_TEXT_PACMO_LEVEL
        DB      0

SCRIPT_PACMO_CAUGHT:
        DB      LCD_ROW1
        DW      LCD_TEXT_PACMO_CAUGHT
        DB      0

SCRIPT_PACMO_GAME_OVER:
        DB      LCD_ROW1
        DW      LCD_TEXT_PACMO_GAME_OVER
        DB      LCD_ROW2
        DW      LCD_TEXT_PACMO_START
        DB      0

SCRIPT_PACMO_COMPLETE:
        DB      LCD_ROW1
        DW      LCD_TEXT_PACMO_COMPLETE
        DB      LCD_ROW2
        DW      LCD_TEXT_PACMO_WAIT
        DB      0

; 15-bit scrolling test bitmap.  Bit 15 is world column 0; bit 1 is column 14.
; This is deliberately a visual pattern, not a colliding maze yet.
; Each row is stored high byte first, low byte second for RENDER_WORLD_TO_BACK.
PACMO_WORLD_ROWS:
        DB      %11111111,%11111110
        DB      %10000010,%00000010
        DB      %10111010,%11101010
        DB      %10001000,%00100010
        DB      %11101011,%10101110
        DB      %10000000,%10000010
        DB      %10111110,%10111010
        DB      %10000010,%00001010
        DB      %10111011,%11101010
        DB      %10001000,%00000010
        DB      %11101110,%11101110
        DB      %10000010,%00000010
        DB      %10111010,%11101010
        DB      %10000000,%00000010
        DB      %11111111,%11111110

; Power-pill coordinates, stored as x,y pairs and terminated by 0xFF.
; These are placed on open cells away from the player start and near broad
; maze regions so they are visible test landmarks before consumption exists.
PACMO_POWER_PILLS:
        DB      1,3
        DB      13,3
        DB      1,11
        DB      13,11
        DB      0xFF

; Enemy respawn candidates, stored as x,y pairs and terminated by 0xFF.
; All entries must be open maze cells.  The respawn routine picks the entry
; with the largest Manhattan distance from the current player position.
PACMO_ENEMY_SPAWNS:
        DB      1,3
        DB      13,3
        DB      1,11
        DB      13,11
        DB      7,1
        DB      7,13
        DB      0xFF
